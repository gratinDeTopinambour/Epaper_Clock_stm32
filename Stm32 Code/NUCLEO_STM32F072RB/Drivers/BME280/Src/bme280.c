 ...
 * 0x77 if SDO pin is connected to 3V3
 * */
...
{
	uint16_t l_shift_addr = BME_ADDR<<1;
...
int BME_Init()
{
	...
	if(id != BME_ID_value) return -1; /* check BME id*/
...
static int BME_Comp()
{
	...
	uint8_t bme_temp_comp[6]; //0x88 -> 0x8D
	...
	uint8_t bme_press_comp[18]; //0x8E -> 0x9F
	...
	uint8_t bme_hum_comp[7]; //0xE1 -> 0xE7
	I2C_Read_nByte(BME_H1, &H1,1); //0xA1
	I2C_Read_nByte(BME_H2_LSB, bme_hum_comp,7);
